...
public class Modele extends Observable {
    ...
    public void tire(int x, int y) {
        if (!p2.getPlateau().getGrillPlayer()[x][y].isDejaTireIci()) {
            ...
            //strategy random et croix ensemble
            /* if(p1.getPlateau().getChange())                        {
                       
                           System.out.println(" cord  x : "+ this.p1.getPlateau().getX()+ " y :"+ this.p1.getPlateau().getY());
                            this.setStrategy("Croix");
                            p2.shoot(p1.getPlateau());
                            p1.getPlateau().setChange(false);
                        }
                        else{
    ...
    public void afficherPlateau(Player p) {

        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 11; j++) {

                System.out.printf(" " + p.getPlateau().getGrillPlayer()[i][j].getId());
            }
            System.out.println(" ");
        }
    }
